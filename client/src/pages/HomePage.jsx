...
import socket, { getState, connection,checkState } from "../utils/socket-methods";
  ...
  const handleSubmit = (e) => {
    ...
    if (name != "") {
     ...
     try {
       ...
       setPlaceholderText(`espere: ${user.username}`);
     } catch (error) {
          ...
          <>
            <div className="h-auto w-2/5 bg-zinc-900 p-4 rounded-lg">
              <p className="text-4xl my-5">
                Muchas Conexiones, intentelo luego... bip bop
              </p>
            </div>
          </>
        ) : (
